...
覆盖分析已完成；G4尚未满足版本验收。完整交叉34/204、未命中170项；Lite32传输20/48，缺28项。AHB5与Classic现有测试的功能覆盖映射仍需补齐。完整矩阵作为诊断保留，不以分析完成给PASS。
...
上述是历史选定日志集合的计数。新版本必测覆盖尚未建立完整映射，不能将其记为0%或100%。
...
blocking_issues:
- 版本必测点与证据映射未齐全，Lite32存在28个传输组合缺口
checks:
- id: coverage-review
  in_scope: true
  status: BLOCKED
