...
class ahb_bridge_scoreboard extends uvm_component;
  ...
  longint unsigned matched_bytes=0,mismatches=0;
  ...
  function void append(ahb_item t,bit is_upstream);
    ...
    while(expected.size() && observed.size()) begin
      ...
      if(equal) matched_bytes++;else begin mismatches++;`uvm_error("AHB-BRIDGE-MISMATCH",$sformatf("expected addr=%h data=%h status=%s observed addr=%h data=%h status=%s",a.addr,a.data,a.status.name(),b.addr,b.data,b.status.name())) end
    end
