...
class ahb_monitor #(int AW=32,DW=32,BW=3,PW=4,MW=0,AU=0,DU=0,RU=0,PROFILE=0, bit SECURE=0,EXCLUSIVE=0,STROBE=0,PARITY=0) extends uvm_monitor;
  ...
  task run_phase(uvm_phase phase);
    forever begin
      ahb_cycle s;
      @(vif.sample_cb);cfg.check_frozen();cycle_count++;s=snapshot();if(cycle_ap.size()) cycle_ap.write(s.duplicate());
      if(cfg.enable_checker) begin
        scb.sample(s);
        foreach(scb.violations[i]) begin
          ahb_violation v;v=scb.violations[i];error_ap.write(v.duplicate());
      ...
      if(s.reset_n!==1) begin
        if(pending!=null) begin pending.lifecycle=ABORTED;pending.status=RESET_ABORT;pending.completed_cycle=cycle_count;transaction_ap.write(pending.duplicate());pending=null;end
        flush_burst("reset");continue;
      end
      if(pending!=null) begin
        if(s.ready===1) begin
          pending.addr=s.address.addr; pending.data=pending.write?s.wdata:s.rdata;pending.strobe=s.strb;
          ...
          transaction_ap.write(pending.duplicate());
