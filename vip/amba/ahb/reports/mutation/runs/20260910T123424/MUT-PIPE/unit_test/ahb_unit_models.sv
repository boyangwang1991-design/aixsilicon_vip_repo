...
task model_checks();
  ...
  config_obj.freeze();config_obj.max_wait=15;check("runtime_policy_not_frozen",config_obj.structural_unchanged());
  config_obj.dw=64;check("structural_mutation_rejected",!config_obj.structural_unchanged() && config_obj.validate()=="structural configuration is frozen");
  config_obj.freeze();check("freeze_cannot_rebaseline_mutation",!config_obj.structural_unchanged());
endtask
