...
class ahb_cycle extends uvm_object;
  ...
  `uvm_object_utils_begin(ahb_cycle)
    ...
    `uvm_field_int(wu,UVM_DEFAULT)
    `uvm_field_int(ru,UVM_DEFAULT)
    `uvm_field_int(bu,UVM_DEFAULT)
    `uvm_field_int(addrchk,UVM_DEFAULT)
    `uvm_field_int(wdchk,UVM_DEFAULT)
    `uvm_field_int(rdchk,UVM_DEFAULT)
    `uvm_field_int(strbchk,UVM_DEFAULT)
    `uvm_field_int(auchk,UVM_DEFAULT)
    `uvm_field_int(wuchk,UVM_DEFAULT)
    `uvm_field_int(ruchk,UVM_DEFAULT)
    `uvm_field_int(buchk,UVM_DEFAULT)
    `uvm_field_int(transchk,UVM_DEFAULT)
    `uvm_field_int(ctrl1,UVM_DEFAULT)
    `uvm_field_int(ctrl2,UVM_DEFAULT)
    `uvm_field_int(protchk,UVM_DEFAULT)
    `uvm_field_int(readychk,UVM_DEFAULT)
    `uvm_field_int(readyoutchk,UVM_DEFAULT)
    `uvm_field_int(respchk,UVM_DEFAULT)
    `uvm_field_int(selchk,UVM_DEFAULT)
  `uvm_object_utils_end
  function new(string name="ahb_cycle");super.new(name);address=ahb_item::type_id::create("address");endfunction
  function ahb_cycle duplicate();ahb_cycle t;$cast(t,clone());return t;endfunction
endclass
class ahb_checker extends uvm_object;
  ...
  function void sample(ahb_cycle s);
    ...
    if(pending!=null) begin
      ...
      if(0 && s.resp!=0 && s.ready===1 && !(previous!=null && previous.ready===0 && previous.resp===s.resp)) flag("AHB-CHECK-ERROR-TWO","missing first non-OKAY cycle",pending);
