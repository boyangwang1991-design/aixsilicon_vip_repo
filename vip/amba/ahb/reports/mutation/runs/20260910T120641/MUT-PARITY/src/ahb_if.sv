...
interface ahb_if #(int AW=32, DW=32, BW=3, PW=4, MW=0, AU=0, DU=0, RU=0,
  ...
  logic [AW-1:0] HADDR;
  logic [1:0] HTRANS;
  logic HWRITE,HMASTLOCK,HSEL,HREADY,HREADYOUT;
  logic [2:0] HSIZE;
  logic [(BW>0?BW:1)-1:0] HBURST;
  logic [(PW>0?PW:1)-1:0] HPROT;
  logic [DW-1:0] HWDATA,HRDATA;
  logic [RW-1:0] HRESP;
  logic HNONSEC,HEXCL,HEXOKAY;
  logic [(MW>0?MW:1)-1:0] HMASTER;
  logic [DW/8-1:0] HWSTRB;
  logic [(AU>0?AU:1)-1:0] HAUSER;
  logic [(DU>0?DU:1)-1:0] HWUSER,HRUSER;
  logic [(RU>0?RU:1)-1:0] HBUSER;
  logic [15:0] HBUSREQ,HGRANT,HSPLIT;
  logic HLOCK;
  logic HTRANSCHK,HCTRLCHK1,HCTRLCHK2,HPROTCHK,HREADYCHK,HREADYOUTCHK,HRESPCHK,HSELCHK;
  logic [(AW+7)/8-1:0] HADDRCHK;
  logic [DW/8-1:0] HWDATACHK,HRDATACHK;
  logic [(DW+63)/64-1:0] HWSTRBCHK;
  logic [(AU>0?(AU+7)/8:1)-1:0] HAUSERCHK;
  logic [(DU>0?(DU+7)/8:1)-1:0] HWUSERCHK,HRUSERCHK;
  logic [(RU>0?(RU+7)/8:1)-1:0] HBUSERCHK;
