...
baseline = yaml.safe_load((RUN / 'evidence/raw/full_s1_w0.yaml').read_text())
...
def run(job):
    ...
    work=RUN/'work/seeds'/profile/str(seed)
    work.mkdir(parents=True)
    with logfile.open('w') as stream:
        stream.write(f'VIP_RUN_SEED={seed}\n');stream.flush()
        try:
            code = subprocess.run(command, cwd=work, stdout=stream, stderr=subprocess.STDOUT, timeout=60).returncode
    ...
    return {'profile': profile, 'seed': seed, 'status': 'PASS' if code == 0 and oracle in output and not bad else 'FAIL', 'exit': code, 'seconds': time.monotonic()-started, 'command': command, 'log': str(logfile.relative_to(RUN))}
...
summary = {'status': 'PASS' if all(c['status'] == 'PASS' for c in cases) else 'FAIL', 'run_id': run_id, 'source_fingerprint': fingerprint, 'tool_version': baseline['tool_version'], 'baseline': 'evidence/raw/full_s1_w0.yaml', 'scope': '100 fixed-seed executions of each profile witness; directed tests, not full per-profile feature closure', 'cases': cases}
(RUN / 'evidence/seed_matrix.yaml').write_text(yaml.safe_dump(summary, sort_keys=False))
