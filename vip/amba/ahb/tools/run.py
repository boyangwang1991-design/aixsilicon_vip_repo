...
p=argparse.ArgumentParser();p.add_argument('tier',choices=['unit','smoke','vectors','negative','extensions','full','compile','burst','error','reset','stress','classic','ral','widths','classic_agent','system']);p.add_argument('--seed',type=int,default=1);p.add_argument('--wait',type=int,default=0);p.add_argument('--count',type=int,default=1000);p.add_argument('--sim',choices=['vcs','xrun'],default='vcs');p.add_argument('--build-tag',default='current');args=p.parse_args()
if not re.fullmatch(r'[A-Za-z0-9_-]+',args.build_tag):p.error('invalid build tag')
...
summary['run_id']=time.strftime('%Y%m%dT%H%M%S',time.gmtime())+'-'+str(time.time_ns())
...
def call(command,cwd,path):
    ...
    with path.open('w') as f:
        f.write(f'VIP_RUN_SEED={args.seed}\n');f.flush()
        try:r=subprocess.run(command,cwd=cwd,stdout=f,stderr=subprocess.STDOUT,timeout=600 if args.tier=='stress' else 180);code=r.returncode
...
if args.sim!='vcs':
    (RUN/'evidence/portability.yaml').write_text(yaml.safe_dump({'status':'NOT_RUN','tool':args.sim,'reason':'xrun is not installed/qualified in this environment'}))
    raise SystemExit(10)
version=subprocess.run(['vcs','-ID'],cwd=RUN/'work/build',capture_output=True,text=True,errors='replace',timeout=30).stdout if args.sim=='vcs' else 'NOT_RUN'
...
for tier in cases:
    ...
    cwd=ROOT/('unit_test' if target=='unit' else 'self_test');build=RUN/'work/build'/args.build_tag/target;build.mkdir(parents=True,exist_ok=True)
    ...
    command=['vcs','-full64','-sverilog','-ntb_opts','uvm-1.2','-timescale=1ns/1ps','-F',str(cwd/filelist),'-top',top,'-Mdir='+str(build/'csrc'),'-o',str(build/'simv')]
    rec={'tier':tier,'seed':args.seed,'wait':args.wait,'command':command,'compile_log':str(logs.relative_to(RUN)/f'{target}_compile.log')}
    code,text,elapsed=call(command,build,logs/f'{target}_compile.log');rec['compile_exit']=code;rec['compile_seconds']=elapsed
    ...
    if code==0 and tier!='compile':
        ...
        command=['/usr/bin/time','-v',str(build/'simv'),'-no_save',f'+ntb_random_seed={args.seed}',f'+WAIT={args.wait}',f'+CASE={tier}',f'+COUNT={args.count}']
        code,text,elapsed=call(command,build,logfile)
        ...
        rec.update(run_command=command,run_exit=code,run_seconds=elapsed,log=str(logfile.relative_to(RUN)),oracle=oracle,status='PASS' if code==0 and oracle in text and not bad else 'FAIL')
        rss=re.search(r'Maximum resident set size \(kbytes\): (\d+)',text)
        if rss:rec['peak_rss_kib']=int(rss[1])
        beats=re.search(r'STRESS_COMPLETIONS (\d+)',text)
        if beats:rec['effective_beats']=int(beats[1])
...
report=RUN/'evidence/raw'/f'{args.tier}_s{args.seed}_w{args.wait}.yaml';report.parent.mkdir(exist_ok=True);report.write_text(yaml.safe_dump(summary,sort_keys=False))
